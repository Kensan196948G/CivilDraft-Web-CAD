...
| 現在の位置付け | **土木特化Web CADの技術プレビュー（v0.1.25）**。ブラウザ内CADコアと土木ドメイン部品は拡充中。Workers APIはP0縦線（Project作成/更新→Drawing作成/更新→Revision作成→Content/数量保存→照査/承認→Export作成→Audit検索）＋断面データ（GET/PUT /sections）を実装し、本番稼働中（`civildraft-web-cad.mirai-dx-platform.com`）。2026-07-21のv0.1.0でpersistX全9ハンドラのNeon永続化配線・監査ログ永続化が本番反映され、Neon migration 0001〜0004適用済み（0003=Neon直接格納・ADR-0014、0004=ID列text整合・ADR-0015）、書き込み系fail-closed暫定措置は撤去済み。2026-07-22のv0.1.1（PR #72・Issue #68恒久対応）でpersistX複合書き込み5種を単一トランザクションへ統合し本番反映済み。2026-07-22のv0.1.2（PR #75・Issue #73恒久対応）でquantity_items孤立item解消も本番反映済み。以降、監査ログhash chain（Issue #61）・数量⇔図形連動（Issue #42）・レイヤーテンプレート・図面健全性チェック（Issue #59）・キーボードショートカット/コマンドパレット（Issue #47）を段階的に本番反映（v0.1.3〜v0.1.17）。2026-08-04のv0.1.18（PR #113/#121、Worker Version `a959db6f`）でactorId偽装対策・リクエストボディ上限等のセキュリティ強化（PR #113）と図面健全性チェック第二弾（Issue #59・数量連動配線バグ修正）を本番反映。2026-08-06のv0.1.19（PR #124〜#134、Worker Version `aa76014d`）でP1全解消。続いてIssue #116（数量明細のstate化・PR #138）とIssue #115（アプリ層レート制限・PR #137）をマージ済み。2026-08-09の金曜開発枠で**Civil-Draw 全機能の吸収統合を完了**（円弧/楕円/スプライン/引出線/改訂雲/平行2線・測距面積・配列・尺度の対話ツール、電子納品チェック、PDF編集・署名マニフェスト、チェックイン/アウト。移行台帳・アーカイブは `docs/migration/`）。Cloudflare Access Application設定とAccess Secret登録は2026-08-09に完了し、Accessログイン保護（カスタムドメイン302）が本番稼働中。2026-08-10にPR #167（ヘルスチェック二層化・CSP・本番実案件表示）をv0.1.23として、PR #169（#62 実案件データ化）とPR #170（監視/SLO・OpenAPI・PWA・現場説明モード・Excel・PDF/A ICC・RFC3161 TSA）をv0.1.24として、PR #172（実図面の改訂更新API配線・断面データAPI・SXF/PDF-A外部検証基盤）をv0.1.25として本番デプロイ（Worker `2fa2cd25`）。総合評価・改善報告書は [comprehensive-evaluation-2026-08-10.md](docs/assessment/comprehensive-evaluation-2026-08-10.md)（改善前74.3→後76.1点・代替率約59→67%） |
...
- 新規図面にはグリッド内のガイド線が初期表示され、ヘッダー「ガイド線」ボタンまたはレイヤーパネルで表示を切替可能
...
| レイヤー管理 | ✅ 実装済み | 表示/非表示・ロック・印刷可否・線幅・追加/削除/リネーム/並替（エディタのレイヤーパネル）。**ロック済みレイヤーは編集コマンド・プロパティ変更・作図から除外**（§6.3 / Issue #40）、PDF出力はprintable=falseを除外、**DXFはレイヤー色/線種を往復で保持**（出力: CONTINUOUS/DASHED/DASHDOT+LTYPE定義、取込: linetype(6)・lock/frozenフラグ(70)を反映）。**工種別レイヤーテンプレート5種**（施工ヤード/仮設/測量/数量根拠/汎用）をワンクリック適用（同名レイヤーは維持・不足分のみ追加） | `src/app/store/editorStore.ts`（LayerSlice）・`src/domain/catalog/layerTemplates.ts`・`src/domain/tools/editGeometry.ts`・`src/domain/pdf/pdfExporter.ts`・`src/domain/dxf/dxfExporter.ts`・`src/domain/dxf/dxfImporter.ts` |
| 作図ツール | ✅ 実装済み | 選択・線・矩形・円・円弧（3点）・楕円・ポリライン・スプライン・改訂雲・平行2線・文字・引出線・寸法・ハッチ・測距/面積（ドラフトプレビュー・自動確定つき状態機械）。Civil-Draw の 5 ツール統合（CALLOUT/CLOUD/ELLIPSE/SPLINE/MLINE）と測距・面積・配列・尺度を完全吸収 | `src/domain/tools/draftGeometry.ts`・ToolSlice |
...
| 電子納品チェック | ✅ 実装済み（2026-08-09） | 国土交通省「工事完成図書の電子納品等要領（令和5年3月版）」＋「電子納品等運用ガイドライン【土木工事編】」に基づくフォルダ構成（DRAWINGF 等 10 種）・命名規則（半角英数・禁則文字）・形式チェック（SXF(P21)/PDF/PDF-A/DXF 警告）・管理項目一覧 CSV・人による最終確認必須ゲート。適合は自動断定しない | `src/domain/edelivery/`・`src/app/pages/EdeliveryPage.tsx` |
| PDF編集・署名 | ✅ 実装済み（2026-08-09 続報） | PDF 結合/分割/回転/透かし・**墨消し（テキスト演算子の物理削除＋黒矩形）**・SHA-256 署名マニフェスト・**PDF/A-1b 指向メタデータ付与（自己宣言・検証必須）**・**PAdES-CMS detached 署名（.p7s）**。制約（ICC 未埋め込み・証明書なし・画像内文字）を UI に明記 | `src/domain/pdf/pdfEdit.ts`・`pdfRedact.ts`・`pdfA.ts`・`pdfSignaturePades.ts`・`src/app/pages/PrintExportPage.tsx` |
| SXF(P21) | 🟡 試作エクスポータ（2026-08-09 続報） | AP202 サブセット（LINE/POLYLINE/CIRCLE・`FILE_SCHEMA('SXF')`）を出力。電子納品チェックシステムでの検証が必須（適合を自動断定しない） | `src/domain/edelivery/sxfP21.ts`・`src/app/pages/PrintExportPage.tsx` |
...
| 現場説明モード | ✅ 実装済み（#58・#178 承認状態結線） | 施工ステップ・数量・根拠図形ハイライト・承認状態・説明用PDF出力を閲覧特化で表示。実 revisionId を持つセッションでは `GET /api/v1/revisions/:id` から承認状態を取得し、取得失敗・未知値は「未取得」を表示（実在しない状態を捏造しない） | `src/app/pages/FieldExplanationPage.tsx`・`src/app/pages/fieldRevisionStatus.ts`・`src/app/App.tsx` |
| 自動保存（IndexedDB） | ✅ 実装・**配線済み** | 起動時に最新下書きを復元、図形/レイヤー変更をデバウンス保存、保存失敗は握り潰さず警告表示 | `src/infrastructure/autosave/`・`App.tsx`（`AutosaveManager`） |
| 認証（Cloudflare Access） | ✅ **実装・本番適用済み** | Access配下のidentity取得層とロール定義に加え、Workers側にJWT二次防御層（RS256署名・iss/aud/exp/nbf検証、JWKS取得、fail-closed）を実装。v0.1.3から actorId は署名検証済みJWTの `email` を採用（`Cf-Access-Authenticated-User-Email` ヘッダー偽装対策）。2026-08-09に Access Application 作成と `CIVILDRAFT_ACCESS_TEAM_DOMAIN`/`CIVILDRAFT_ACCESS_AUD` のシークレット登録を完了し、カスタムドメインは Access ログイン保護（302→cloudflareaccess.com）、不正JWTは401 fail-closed で本番稼働中。**2026-08-12（#177）から SPA 側でも identity からロールを解決し、viewer（閲覧者）では編集系ナビ・作成ボタン・編集系画面を非表示、サイドバーに利用者名とロールバッジを表示（取得不可時は開発/テスト engineer・本番 viewer へフォールバック）** | `src/infrastructure/auth/accessIdentity.ts`・`src/workers/accessJwt.ts`・`src/workers/index.ts`・`src/app/App.tsx`・`src/app/layout/Sidebar.tsx`・`tests/unit/workers/accessJwt.test.ts`・`tests/unit/workers/auditHardening.test.ts` |
...
│  └─ pages/        ✅ 業務画面（CadEditorPage の図形プロパティ入力・ラベル/スタイル定数は `cadEditor/` へ抽出済み・#179）
├─ infrastructure/  ✅一部
│  ├─ autosave/     ✅ IndexedDB 自動保存（配線済み）
│  ├─ auth/         ✅ Cloudflare Access identity（AppShell でロール解決・viewer UI 反映・#177）
...
| 0+ | ✅ 完了（2026-08-09） | **Civil-Draw 完全吸収**: 必須 37 機能の実装単位突合・100% 判定、円弧/楕円/スプライン/引出線/改訂雲/平行2線・測距面積・配列・尺度を統合、電子納品・PDF 編集・チェックイン/アウトを新規実装。移行台帳・統合報告書・削除判定チェックリスト・アーカイブ一式は `docs/migration/` |
...
| 2026-08-06 | 🚀 **v0.1.20 本番デプロイ**（PR #137/#138/#141〜#147・/goal事前承認）: #24スナップ配線・#25回転二重適用解消・#23 Arc掃引規約・#26コード分割（初期ロード約18%減）・#120キャンバスA11y・#115アプリ層レート制限・#116数量state化・409競合UX・監査CSV注入対策・ops改善（503文言/CI SHA固定/カバレッジ閾値/CHANGELOG等）。main `9066e8c` CI全チェックsuccess・ローカル1365テストPASS → wrangler deploy（Worker Version `22ee0438`）。スモーク全PASS（[Release v0.1.20](../../releases/tag/v0.1.20)） |
| 2026-08-06 | 🚀 **v0.1.21 本番デプロイ**（PR #149〜#154・/goal事前承認）: #46 A3表題欄テンプレート・#39 一括プロパティ編集・#44 測点GeoJSON出力・#41 記号4種追加・#47 CADコマンドライン＋ショートカット一覧・#43 ファイル互換方針。main `2320dfc` CI全チェックsuccess・ローカル1379テストPASS → wrangler deploy（Worker Version `a33f6529`）。スモーク全PASS（[Release v0.1.21](../../releases/tag/v0.1.21)） |
| 2026-08-06 | 🚀 **v0.1.22 本番デプロイ**（PR #156〜#158・/goal事前承認）: #44 測点のLandXML 1.2出力・#60 移行アシスタント（未対応形式の案内＋DXFイシュー対処提案）・#39残 分解（ポリライン/矩形）と結合（同一直線）。main `dcabdc6` CI全チェックsuccess・ローカル1395テストPASS → wrangler deploy（Worker Version `fbf65869`）。スモーク全PASS（[Release v0.1.22](../../releases/tag/v0.1.22)） |
| 2026-08-09 | 🏗️ **Civil-Draw 完全吸収統合**（金曜開発枠）: 円弧・楕円・スプライン・引出線・改訂雲・平行2線・測距/面積・配列複写・尺度の対話ツールを実装（CloudGeometry/MLineGeometry 新設、全幾何エンジン・Canvas・PDF・DXF へ一貫適用）。電子納品チェック画面・PDF 編集（結合/分割/回転/透かし/墨消し）・署名マニフェスト・チェックイン/アウトを新規実装。移行台帳・統合報告書・削除判定チェックリスト・アーカイブ（git bundle・Issue/PR/Release 全量）を `docs/migration/` へ整備。単体テスト 1444 pass・lint/typecheck/build PASS |
| 2026-08-09 | 🚀 **v0.1.23 前段（Access 本番適用・残課題対応 第2弾）**（PR #163/#165 相当）: Cloudflare Access アプリ作成＋`CIVILDRAFT_ACCESS_TEAM_DOMAIN`/`CIVILDRAFT_ACCESS_AUD` シークレット登録→Worker Version `f2edb5b2` で本番反映。SXF(P21) 円弧 TRIMMED_CURVE 化・PAdES ByteRange 埋め込み署名・PDF/A XMP/OutputIntent・画像墨消しツール・migration 0007（チェックアウト永続化）を実装。テスト 1463 pass |
| 2026-08-10 | 🚀 **v0.1.23 本番デプロイ**（PR #167・/goal 事前承認）: 本番ヘルスチェック二層化（#164 復旧・`[health] OK`）・CSP 追加・本番モードのホーム実案件表示（#62 部分解消・`?demo=1` で従来表示）。main `d5f32c8` CI 全チェック success・テスト 1466 pass → wrangler deploy（Worker Version `738799f4`）。スモーク全PASS（[Release v0.1.23](../../releases/tag/v0.1.23)）。総合評価・改善報告書 [comprehensive-evaluation-2026-08-10.md](../assessment/comprehensive-evaluation-2026-08-10.md) |
| 2026-08-10 | 🛠️ **Issue #62 残対応（サンプルデータ除去・実案件データ運用化）**（PR #169）: 案件詳細を Workers API 実データ版（ProjectDetailCloudPage）へ切替。ホームの実案件選択→案件詳細遷移・図面作成/案件編集（PATCH/POST）・メンバー一覧・ローディング/空状態/エラー表示を実装し、本番モードのサンプル表示（断面サンプル・測点サンプルCSV・出力履歴・監査フォールバック・照査承認デモ改訂・電子納品初期値・CAD編集デフォルトセッション・設定画面・印刷画面）を非表示化（`?demo=1` 時のみ開発用に維持）。API クライアントへ `getProject` / `listProjectDrawings` / `listProjectMembers` / `getRevision` / `updateProject` を追加。ローカル 1491 テスト PASS・lint/typecheck/build/audit PASS（ローカル E2E は Chromium SIGTRAP の環境要因で CI 依存） |
| 2026-08-10 | 🚀 **v0.1.24 本番デプロイ**（PR #170・/goal 事前承認）: 監視通知（Slack・SLO週次集計）、OpenAPI 仕様書、Neon 実接続 CI（secret 設定時）、PWA（manifest/SW/アイコン・モバイルサイドバー）、現場説明モード（#58・CAD編集画面に切替ボタン）、Excel 出力（数量/図面リスト/サマリー・数式インジェクション対策）、PDF/A（公式 sRGB2014 ICC 埋め込み・verapdf 検証）、RFC 3161 TSA クライアント（PAdES 準備）、SXF 適合状況文書。main `b193fd2` CI 全チェック success・テスト **1508 pass** → wrangler deploy（Worker Version `001eb27f`）。スモーク全PASS（[Release v0.1.24](../../releases/tag/v0.1.24)） |
| 2026-08-10 | 🚀 **v0.1.25 本番デプロイ**（PR #172・/goal 事前承認）: 実図面の改訂更新 API 配線（既存図面の内容・数量読込＋楽観ロック付き共有更新）、断面データ API（GET/PUT `/api/v1/revisions/:id/sections`・migration 0008・縦横断管理画面の実案件連携）、SXF/PDF-A 外部検証基盤（PDF/A サンプル生成・SXF 構造検証・`docs/edelivery-validation.md`）。main `8bd34a3` CI 全チェック success・テスト **1515 pass** → wrangler deploy（Worker Version `2fa2cd25`）。スモーク全PASS（[Release v0.1.25](../../releases/tag/v0.1.25)） |
| 2026-08-12 | 🛠️ **残課題 4 件実装**（#177〜#180）: 閲覧ロールUI導線（AppShell で Access identity からロール解決・viewer は編集系ナビ/作成ボタン/編集系画面を非表示・サイドバーにロールバッジ）、現場説明モードの承認状態結線（実 revisionId で `GET /api/v1/revisions/:id`・失敗時「未取得」）、CadEditorPage 分割（図形プロパティ入力・ラベル/スタイル定数を `src/app/pages/cadEditor/` へ抽出・2,559→2,196 行）、利用者向け操作マニュアル（`docs/operations/user-manual.md`・README リンク）。テスト **1547 pass**・lint 0/0・typecheck/build PASS |
...
| [#102](../../pull/102) | **v0.1.14**: 線種のDXF往復強化（#40残）: linetype(6)・lock/frozenフラグ(70)取込 | ✅ マージ済み（2026-08-02・admin squash `e9541f1`）→ v0.1.14本番デプロイ済み |
...
| [#141](../../pull/141) | 共有保存の楽観ロック競合UX（409/428・最新版再読込導線） | ✅ マージ済み（2026-08-06・`729fab0`）→ v0.1.20 |
| [#142](../../pull/142) | **Issue #24**: スナップエンジン配線＋スナップ設定UI（SnapMarker可視化） | ✅ マージ済み（2026-08-06・`6425fa5`）→ v0.1.20 |
| [#143](../../pull/143) | 監査CSVの数式インジェクション対策（escapeCsvCell） | ✅ マージ済み（2026-08-06・`20a81d1`）→ v0.1.20 |
| [#144](../../pull/144) | **Issue #25**: rectangle/ellipse 90°回転の二重適用解消 | ✅ マージ済み（2026-08-06・`6aa967f`）→ v0.1.20 |
| [#145](../../pull/145) | **Issue #23**: Arc掃引方向規約確定・フィレット弧の小さい弧化 | ✅ マージ済み（2026-08-06・`25b6965`）→ v0.1.20 |
| [#146](../../pull/146) | **Issue #26**: 業務ページのReact.lazyコード分割（初期ロード約18%減） | ✅ マージ済み（2026-08-06・`f9d9875`）→ v0.1.20 |
| [#147](../../pull/147) | **Issue #120**: キャンバスA11y（フォーカス・矢印パン・ライブリージョン） | ✅ マージ済み（2026-08-06・`9066e8c`）→ v0.1.20 |
| [#149](../../pull/149) | **Issue #46**: A3表題欄テンプレート（図面枠カテゴリ） | ✅ マージ済み（2026-08-06・`bc6c095`）→ v0.1.21 |
| [#150](../../pull/150) | **Issue #39**: 複数選択の一括プロパティ編集（1 undo） | ✅ マージ済み（2026-08-06・`d51b50d`）→ v0.1.21 |
| [#151](../../pull/151) | **Issue #44**: 測点のGeoJSON出力（RFC 7946） | ✅ マージ済み（2026-08-06・`176cc22`）→ v0.1.21 |
| [#152](../../pull/152) | **Issue #41**: 土木現場用記号4種追加 | ✅ マージ済み（2026-08-06・`0c56a5d`）→ v0.1.21 |
| [#153](../../pull/153) | **Issue #47**: CADコマンドライン＋ショートカット一覧 | ✅ マージ済み（2026-08-06・`8d80e26`）→ v0.1.21 |
| [#154](../../pull/154) | **Issue #43**: ファイル互換方針の確定 | ✅ マージ済み（2026-08-06・`2320dfc`）→ v0.1.21 |
...
| [#161](../../pull/161) | Civil-Draw参照をアーカイブへ変更（削除準備）・PR #160実績追記 | ✅ マージ済み（2026-08-09・`2f051a3`） |
| [#162](../../pull/162) | docs: Civil-Draw削除実行記録（削除チェックリスト・state.json同期） | ✅ マージ済み（2026-08-09・`6067f21`） |
| [#163](../../pull/163) | 残課題の開発対応（チェックアウト永続化・墨消し物理削除・PDF/A・PAdES署名・SXF試作） | ✅ マージ済み（2026-08-09・`1ee0bf6`） |
| [#165](../../pull/165) | 残課題対応 第2弾（Access本番適用・Neon移行手順・SXF円弧・PAdES埋め込み署名・画像墨消しツール） | ✅ マージ済み（2026-08-09・`023ab13`）→ v0.1.23 |
| [#166](../../pull/166) | docs: 残課題の詳細手順書（HTML）を追加 | ✅ マージ済み（2026-08-09・`d6e2682`） |
| [#167](../../pull/167) | **本番監視・セキュリティ・データ品質の緊急改善**（#164/#62: ヘルスチェック二層化・CSP・本番実案件表示） | ✅ マージ済み（2026-08-10・`d5f32c8`）→ v0.1.23 |
| [#169](../../pull/169) | **Issue #62: サンプルデータ除去・実案件データ運用化**（案件詳細の API 実データ化・本番モードのサンプル非表示化・API クライアント拡張） | ✅ マージ済み（2026-08-10・`a4fdb0a`）→ v0.1.24 |
| [#170](../../pull/170) | **最優先改善の一括実装**: 監視通知/SLO・OpenAPI・Neon実接続CI・PWA・現場説明モード（#58）・Excel出力・PDF/A ICC・RFC3161 TSA | ✅ マージ済み（2026-08-10・`b193fd2`）→ v0.1.24 |
| [#172](../../pull/172) | **実図面の改訂更新API配線・断面データAPI（migration 0008）・SXF/PDF-A外部検証基盤** | ✅ マージ済み（2026-08-10・`8bd34a3`）→ v0.1.25 |
...
| [`docs/operations/user-manual.md`](./docs/operations/user-manual.md) | 利用者向け操作マニュアル（ロール別・作図/数量/照査/出力/電子納品・FAQ） |
