...
| P2 | E2E/性能試験 | 🟡 基礎整備済み: Playwright E2E（作成→作図→自動保存→復元→DXF/PDF出力・照査承認）と性能ベンチマーク（10k空間索引・10MB DXF取込・10k DXF出力・2k PDF出力・1k Undo/Redo）を追加。`npm run e2e` / `npm run perf` で実行し、閾値超過はCI warningとして監視（#45/#63）。残: DXF取込UI配線（#118）後の10k図形ブラウザ描画・クラウド保存E2E |
