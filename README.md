...
| 2026-08-10 | 🛠️ **Issue #62 残対応（サンプルデータ除去・実案件データ運用化）**（PR #169）: 案件詳細を Workers API 実データ版（ProjectDetailCloudPage）へ切替。ホームの実案件選択→案件詳細遷移・図面作成/案件編集（PATCH/POST）・メンバー一覧・ローディング/空状態/エラー表示を実装し、本番モードのサンプル表示（断面サンプル・測点サンプルCSV・出力履歴・監査フォールバック・照査承認デモ改訂・電子納品初期値）を非表示化（`?demo=1` 時のみ開発用に維持）。API クライアントへ `getProject` / `listProjectDrawings` / `listProjectMembers` / `getRevision` / `updateProject` を追加。ローカル 1489 テスト PASS・lint/typecheck/build/audit PASS（ローカル E2E は Chromium SIGTRAP の環境要因で CI 依存） |
