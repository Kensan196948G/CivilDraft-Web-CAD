...
| PDF編集・署名 | ✅ 実装済み（2026-08-09） | PDF 結合/分割/回転/透かし/墨消し（視覚的・制約明記）・SHA-256 署名マニフェスト（電子署名法上の署名ではない旨を明記）。PAdES/PDF-A 変換は課題 | `src/domain/pdf/pdfEdit.ts`・`src/domain/pdf/pdfSignature.ts`・`src/app/pages/PrintExportPage.tsx` |
| チェックイン/アウト | 🟡 ドメイン+UI実装（2026-08-09） | 排他編集の所有権モデル（draft/returned のみ取得可・承認後改変防止）とエディタ UI（localStorage 永続化）。共有版は楽観ロック 409 と併用。サーバー横断ロック永続化は課題 | `src/domain/revisions/checkout.ts`・`src/app/pages/CadEditorPage.tsx` |
