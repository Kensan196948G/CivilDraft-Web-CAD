...
| 数量⇔図形連動 | 🟡 **ハイライト連動実装（#42 第一弾）** | 数量集計画面で明細を選択 → 根拠図形（quantity sources の geometryId）を「図面で確認」で CAD 編集画面へ遷移し、キャンバス上をオレンジの破線枠でハイライト。図形→数量行の逆方向（クリック→明細ハイライト）は次フェーズ | `src/app/store/editorStore.ts`（highlightedGeometryIds）・`src/app/canvas/CanvasStage.tsx`・`src/app/pages/QuantitySummaryPage.tsx` |
