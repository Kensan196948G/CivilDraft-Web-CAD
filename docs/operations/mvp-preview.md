...
4. CAD 編集: 図面行の「CAD編集で開く」で、種別ごとのサンプル図形（施工ヤード図=敷地/道路/クレーン、仮設計画図=矢板/切梁、土工断面図=現況/計画地盤/ハッチ、数量根拠図=面積寸法/引出線）が初期表示される。作図（線/矩形/円/ポリライン等）・レイヤー・グリッド・Undo/Redo・DXF 取込/出力・自動保存（IndexedDB）復旧を即操作可能
