...
> **対象フェーズ: リリース前検証段階。公開後の本番ロールバックは人間決裁後に具体化する。**
>
> 本プロジェクトは Vite + React SPA と Workers API 検証実装を持ちます。
> 初回公開構成は Cloudflare Workers Static Assets に確定していますが、外部公開、Workers API本番経路、
> Neon PostgreSQL / Object Storage 接続は未承認です。
> したがって現段階のロールバックは「**コード（Git）の切り戻し**」と
> 「**成果物（`dist/`）の再生成・再配置準備**」に限定します。
> 本番DBのデータロールバックは、DB接続が承認されるまで発生しません。
...
| 本番配置後の不具合 | 静的配信の配置版を確認し、直前成果物へ戻す | ⚠️ 公開後に具体化 |
...
> ✅ **現在: 本番DBのデータロールバックは不要。**
>
> Workers API は検証用インメモリ実装であり、本番DBには接続していない。
> 編集中データはブラウザの **IndexedDB にローカル保存**される。
...
Neon PostgreSQL を本番接続した後は、以下を本節に具体化する（現段階では枠のみ）。

| 項目 | 本番接続後の想定 | 状態 |
| --- | --- | --- |
| スキーマ変更の検証 | Neon **dev ブランチ**で隔離検証してから本番反映 | ⚠️ 本番接続時に確定 |
| マイグレーション適用 | `prepare_database_migration` → dev で確認 → 人間承認で本番反映 | ⚠️ 本番接続時に確定 |
| データ切り戻し | Neon のブランチ / PITR（ポイントインタイムリカバリ）方針 | ⚠️ 本番接続時に確定 |
