// FORKED from design-sync's lib/bundle.mjs (declared in cfg.libOverrides).
// Two targeted fixes for this repo, both isolated to functions below —
// everything else is an unmodified copy of the bundled original:
//
// 1. tsconfigPathsPlugin: its `exts` candidate loop used existsSync(), which
//    returns true for directories as well as files. This repo's @/domain/*
//    aliases are barrel directories (e.g. `src/domain/sections/index.ts`),
//    so the bare '' extension candidate matched the directory itself before
//    the loop ever reached the `/index.ts` candidate later in the same
//    list, and esbuild failed with "Cannot read file ...: is a directory".
//    Fix: only accept a candidate that statSync().isFile().
//
// 2. sharedBuildOptions: this repo intentionally keeps
//    src/app/store/editorStoreContext.ts (the Context value, react-refresh
//    safe) separate from EditorStoreContext.tsx (the Provider component) —
//    a standard Vite/React Fast Refresh split, not a naming mistake.
//    esbuild's default resolveExtensions tries `.tsx` before `.ts`; for the
//    bare import `./editorStoreContext` that means it first probes
//    `editorStoreContext.tsx`, which doesn't exist but case-insensitively
//    collides with the real `EditorStoreContext.tsx` — esbuild warns
//    [different-path-case] and then fails trying to read the literal-case
//    path. Fix: list `.ts` before `.tsx` so the exact-case match resolves
//    before the collision is ever probed.
//
// See .design-sync/NOTES.md for the full write-up.
...
export function tsconfigPathsPlugin(tsconfigPath) {
  ...
  // FORK: a bare stem (ext === '') exists() for directories too — a barrel
  // alias target (e.g. @/domain/sections → src/domain/sections/) must fall
  // through to its /index.ts candidate instead of matching the directory.
        ...
        for (const r of rules) {
          ...
          for (const t of r.targets) {
            ...
            for (const ext of exts) {
              if (existsAsFile(stem + ext)) return { path: stem + ext };
            }
          }
        }
        return undefined;
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    plugins,
    metafile: true,
    // FORK: this repo pairs editorStoreContext.ts (Context value) with
    // EditorStoreContext.tsx (Provider) — a standard react-refresh split,
    // not a collision to rename away. esbuild's default order tries .tsx
    // before .ts, which makes the bare import './editorStoreContext' probe
    // a literal-case '.tsx' path first, case-insensitively collide with the
    // real EditorStoreContext.tsx, and fail reading it. Listing .ts first
    // lets the exact-case match resolve before that collision is probed.
