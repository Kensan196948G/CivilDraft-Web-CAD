...
- `/goal` コマンド駆動の自律継続開発（Codex v2.1.159+ 公式機能）
...
```json
"hooks": {
  "TeammateIdle": [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node .Codex/claudeos/scripts/hooks/teammate-idle-gate.js" }] }],
  "TaskCreated":  [{ "matcher": "*", "hooks": [{ "type": "command", "command": "node .Codex/claudeos/scripts/hooks/task-created-gate.js" }] }],
  "TaskCompleted":[{ "matcher": "*", "hooks": [{ "type": "command", "command": "node .Codex/claudeos/scripts/hooks/task-completed-gate.js" }] }]
}
```
...
| `r` | 選択エージェントを再実行 |
| `s` | スクリプトをコマンドとして保存（`.Codex/workflows/` または `~/.Codex/workflows/`） |

**ワークフロー保存場所:**

| パス | スコープ |
|---|---|
| `.Codex/workflows/<name>.js` | プロジェクト共有（git でチーム全員に配布） |
| `~/.Codex/workflows/<name>.js` | ユーザー個人（全プロジェクトで利用可） |

保存したワークフローは `/` でオートコンプリート候補として表示される。

**ワークフローの keyword トリガー:**
プロンプトに `workflow` という単語を含めるだけで、Codex がそのタスク用ワークフローを自動作成する。
...
- headless セッションでプラグイン MCP が 401/403 を返す場合は認証失効。`Codex mcp list` で状態確認し、
...
Codex 公式の multi-agent cloud review を行う。
...
| `Codex plugin init <name>` | プラグイン scaffold 生成 | 新プラグイン作成時 |
| `Codex plugin prune` | 孤立依存関係の削除 | プラグイン整理時 |
...
{
  "type": "command",
  "command": "node .Codex/claudeos/scripts/hooks/pre-commit-gate.js",
...
| オプション | 説明 |
|---|---|
| `continueOnBlock: true` | フックがブロックした際に理由を Codex にフィードバック（修正判断に活用） |
...
| モデル | ID | 特徴 |
|---|---|---|
| **Opus 4.8** | `Codex-opus-4-8` | xhigh effort デフォルト、Fast Mode で 2.5× 高速 |
| **Sonnet 5** | `Codex-sonnet-5` | 1M context、バランス型（**現デフォルト**、2026-08-31 までプロモ価格） |
| Haiku 4.5 | `Codex-haiku-4-5-20251001` | 軽量・高速（/goal 達成判定用） |
