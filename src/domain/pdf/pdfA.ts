 ...
 *   （DestOutputProfile の ICC プロファイル埋め込みは省略 → 検証ツールで警告対象）。
...
export async function applyPdfAMetadata(
  ...
  try {
    ...
    // OutputIntent（GTS_PDFA1）。DestOutputProfile（ICC）は未埋め込み。
    ...
    return {
      ...
      value: {
        ...
        issues: [
          ...
          'OutputIntent に ICC プロファイル（DestOutputProfile）を含まないため、検証ツールでは警告が出ます。',
        ],
      },
    }
  } catch {
