...
2. **隔離検証**: dev ブランチ上で `0001_initial_schema.sql` を適用し（`run_sql` / `prepare_database_migration`）、
...
- DB 導入後のスキーマ切り戻し・データ復旧（Neon ブランチ / PITR）方針は
  `rollback-procedure.md` §4.1 に具体化する（現段階は参照枠）。
